...
class IngredientController {
	...
	static async addIngredient(req: Request, res: Response): Promise<void> {
		const { food_description, nutrients, food_group_id, food_group_name } =
			req.body;

		if (!food_description) {
			res.status(400).send(
				"Invalid request body: 'food_description' is required"
			);
	...
	static async editIngredient(req: Request, res: Response): Promise<void> {
		...
		try {
			...
			if (food_description !== undefined)
				updateData.food_description = food_description;
			if (nutrients !== undefined) updateData.nutrients = nutrients;
			if (food_group_id !== undefined)
				updateData.food_group_id = food_group_id;
			if (food_group_name !== undefined)
				updateData.food_group_name = food_group_name;

			const updated = await Ingredient.findByIdAndUpdate(id, updateData, {
				new: true,
				runValidators: true,
			});
				...
				const batchPromises = batch.map(async (food: any) => {
					try {
						...
						const detailedFood = await CNFService.getFood(
							food.food_code,
							nutrient_groups
								? Array.isArray(nutrient_groups)
									? nutrient_groups.map(Number)
									: [Number(nutrient_groups)]
								: undefined
						);
						...
						logger.error(
							`Error processing food ${food.food_code}:`,
							error
						);
						results.errors++;
					}
